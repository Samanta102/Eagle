...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-edit"></i> Editar Cotización #{{ $cotizacion->id_cotizacion }}</h1>
        </div>
            ...
            <form action="{{ route('cotizaciones.update', $cotizacion->id_cotizacion) }}" method="POST">
                    ...
                    <select class="form-control" name="id_usuario" required>
                        ...
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id_usuario }}" 
                                {{ $cotizacion->id_usuario == $usuario->id_usuario ? 'selected' : '' }}>
                    ...
                    <select class="form-control" name="id_diagnostico" required>
                        ...
                        @foreach($diagnosticos as $diagnostico)
                            <option value="{{ $diagnostico->id_diagnostico }}" 
                                {{ $cotizacion->id_diagnostico == $diagnostico->id_diagnostico ? 'selected' : '' }}>
                    ...
                    <select class="form-control" name="id_forma_pago" required>
                        ...
                        @foreach($formas_pago as $fp)
                            <option value="{{ $fp->id_forma_pago }}" 
                                {{ $cotizacion->id_forma_pago == $fp->id_forma_pago ? 'selected' : '' }}>
                                {{ $fp->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control" name="total" step="0.01" min="0" 
                               value="{{ old('total', $cotizacion->total) }}" required>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" name="fecha_emicion" 
                           value="{{ old('fecha_emicion', $cotizacion->fecha_emicion) }}" required>
                </div>
