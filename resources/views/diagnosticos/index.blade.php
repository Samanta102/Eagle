@extends('layouts.app')
@section('title', 'Lista de Diagnósticos | Taller Mecánico')
@section('content')
    ...
    <div class="main-content">
        <div style="width: 100%; max-width: 1200px; margin: 30px auto 20px auto; display: flex; justify-content: space-between; align-items: center;">
            <h1 style="color: #2c3e50; font-weight: 600; margin: 0; display: flex; align-items: center; gap: 10px;">
                <i class="fas fa-clipboard-list"></i> Listado de Diagnósticos
            </h1>
            <a href="{{ route('diagnosticos.create') }}" style="background: #3498db; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: 500; display: inline-flex; align-items: center; gap: 8px;">
            ...
            <table style="margin: 0 auto; border-collapse: collapse; min-width: 1200px; background: #fff; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.07); overflow: hidden;">
                <thead>
                    <tr style="background: #2c3e50; color: #ffffff;">
                        ...
                        <th class="table-padding">Forma de Pago</th>
                        <th class="table-padding">Costo Total</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #e0e0e0;">
                        <td class="table-padding-td">{{ $diagnostico->id_diagnostico }}</td>
                        <td class="table-padding-td">
                            {{ $diagnostico->usuario->nombre_usuario ?? 'Sin nombre' }}
                            {{ $diagnostico->usuario->apellido ?? '' }}
                        </td>
                        <td class="table-padding-td">{{ Str::limit($diagnostico->descripcion, 50) }}</td>
                        <td class="table-padding-td">
                            <span class="badge badge-success">
                                {{ $diagnostico->formaPago->nombre ?? 'Sin forma de pago' }}
                            </span>
                        </td>
                        <td class="table-padding-td">$ {{ number_format($diagnostico->costo_total, 2) }}</td>
                        <td class="table-padding-td">
                            <a href="{{ route('diagnosticos.edit', $diagnostico->id_diagnostico) }}" style="background: #f39c12; color: #fff; text-decoration: none; padding: 7px 14px; border-radius: 4px; font-size: 0.95em; margin-right: 6px; display: inline-block;">
                                <i class="fas fa-edit"></i> Editar
                            </a>
                            <form action="{{ route('diagnosticos.destroy', $diagnostico->id_diagnostico) }}" method="POST" style="display:inline;" onsubmit="return confirm('¿Estás seguro de eliminar este diagnóstico?');">
                                @csrf
                                @method('DELETE')
