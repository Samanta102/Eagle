...
<div class="container">
    ...
    <form method="POST" action="{{ route('citas.update', $cita) }}">
        ...
        <div class="form-group">
            ...
            <select class="form-select" name="id_usuario" id="id_usuario" required>
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id_usuario }}" {{ old('id_usuario', $cita->id_usuario) == $usuario->id_usuario ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <select class="form-select" name="id_patineta" id="id_patineta" required>
                @foreach($patinetas as $patineta)
                    <option value="{{ $patineta->id_patineta }}" {{ old('id_patineta', $cita->id_patineta) == $patineta->id_patineta ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="motivo" id="motivo" rows="4" required>{{ old('motivo', $cita->motivo) }}</textarea>
        </div>
